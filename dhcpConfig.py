...
class DhcpdGlobalConfig:
    ...
    def to_string(self) -> str:
        return (
            ...
            f"ddns-updates off;\n"
            f"do-forward-updates off;\n"
            f"default-lease-time 7200;\n"
            f"max-lease-time 7200;\n"
            f"deny unknown-clients;\n"
            f"authoritative;\n"
            f"DHCPD_INTERFACE={self.port};\n\n" if self.port else f"\n\n"
        )
...
class DhcpConfig:
    _global_config: DhcpdGlobalConfig = None
    ...
    def add_global(self, port):
        self._global_config = DhcpdGlobalConfig(port)

    def to_string(self) -> str:
        config_str = ""
        if self._global_config:
            config_str += self._global_config.to_string()
        for subnet in self._subnet_configs:
...
def render_dhcpd_conf(mac: str, ip: str, name: str, port: str) -> None:
    ...
    dhcp_config.add_global(port)

    dhcp_config.write_to_file()


def configure_dhcpd(node: NodeConfig, port: str) -> None:
    logger.info("Configuring dhcpd entry")

    render_dhcpd_conf(node.mac, str(node.ip), node.name, port)
    lh = host.LocalHost()
